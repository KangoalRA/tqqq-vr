...
c3.metric("최종 현금 (Pool)", f"${final_pool:,.0f}", f"+${add_usd:,.0f} 신규")
...
with tab1:
    ...
    with col_buy:
        st.subheader("🔵 2주 지정가 잔량 예약")
        st.write("다음 업데이트 전까지 **2주간 유지될 그물**을 치세요.")
        limit = final_pool * pool_cap
        buy_table = []
        for i, r in enumerate([0.98, 0.96, 0.94, 0.92, 0.90]):
            p = curr_p * r
            q = int((limit/5)/p)
            if q >= 1: buy_table.append({"단계": f"{i+1}차", "가격": f"${p:.2f}", "총수량": f"{q}주"})
        st.table(pd.DataFrame(buy_table))
        st.info("⚠️ **주의:** '지정가' + '잔량' 조건으로 2주 기간 예약을 거세요.")
    ...
    with col_sell:
        ...
        if qty > 0:
            ...
            if curr_p >= target_p:
                ...
                st.error(f"🚨 **밴드 상단 돌파!** 약 {int(excess/curr_p)}주 매도하세요.")
            else:
...
with tab2:
    if not df.empty:
        ...
        if not plot_df.empty:
            ...
            fig.update_layout(height=500, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(tickformat='%y-%m-%d'))
